...
export const EnablePatient = () => {
  ...
  const fetchPatients = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/api/admin/fetch-disable-patients', {
  ...
  const enablePatient = async (id) => {
    try {
      await axios.post(`/api/admin/enable-patient/${id}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
  
      window.location.reload(); // Reload the page on success
    } catch (error) {
      console.error('Error enabling patient:', error);
    }
  };
  ...
  const handleSearch = async (e) => {
    ...
    if (searchValue.length > 2) {
      setLoading(true);
      try {
        const response = await axios.get('/api/admin/search-disable-patients', {
          headers: { Authorization: `Bearer ${token}` },
  ...
  <h2 className="text-center mb-4">Disabled Patients</h2>
    ...
    <div className="list-group mb-3" style={{ maxHeight: "200px", overflowY: "auto" }}>
      {suggestions.map((suggestion) => (
        ...
        >
          ...
          <br />
          <span>Phone: {suggestion.phone}</span>
        </div>
      ))}
    </div>
  )}
  ...
  <ul className="list-group">
    {selectedItem ? (
      <li key={selectedItem.id} className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <button className="btn btn-success btn-sm" onClick={() => enablePatient(selectedItem.id)}>
          Enable
        </button>
      </li>
    ) : (
      ...
      patients.map((patient) => (
        <li key={patient.id} className="list-group-item d-flex justify-content-between align-items-center">
          ...
          <button className="btn btn-success btn-sm" onClick={() => enablePatient(patient.id)}>
            Enable
          </button>
        </li>
      ))
