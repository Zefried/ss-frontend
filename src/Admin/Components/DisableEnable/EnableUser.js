...
export const EnableUser = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/api/admin/fetch-disable-user', {
  ...
  const enableUser = async (id) => {
    try {
      await axios.post(`/api/admin/enable-user/${id}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSearch = async (e) => {
    ...
    if (searchValue.length > 2) {
      setLoading(true);
      try {
        const response = await axios.get('/api/admin/search-disable-user', {
          headers: { Authorization: `Bearer ${token}` },
  ...
  return (
    <div className="container mt-5">
      <h2 className="text-center mb-4">Disabled Users</h2>
        ...
        <div className="list-group">
          {suggestions.map((suggestion) => (
            ...
            >
              ...
              <br />
              <span>Email: {suggestion.email}</span>
            </div>
          ))}
        </div>
      )}
        ...
        {selectedItem ? (
          ...
          >
            ...
            <button
              ...
              onClick={() => enableUser(selectedItem.id)}
            >
              Enable
            </button>
          </li>
